...
func TestSolicitarCompraEReservarInsumosHandler(t *testing.T) {
	const corpo = `{"ordemServicoId":"10000000-0000-0000-0000-000000000001","fornecedorId":"20000000-0000-0000-0000-000000000001","itens":[{"itemId":"30000000-0000-0000-0000-000000000001","quantidade":2.5}]}`
	...
	t.Run("retorna 201", func(t *testing.T) {
		fake := &processarRepositorioFake{resultado: application.ResultadoCompraReserva{
			...
			InsumosReservados:  []application.ItemReservado{{ItemID: "30000000-0000-0000-0000-000000000001", Quantidade: json.Number("2.5")}},
		}}
		resposta := executarProcessamento(fake, corpo, "40000000-0000-0000-0000-000000000001")

		if resposta.Code != http.StatusCreated || !strings.Contains(resposta.Body.String(), `"insumosReservados"`) {
			t.Fatalf("status=%d body=%s", resposta.Code, resposta.Body.String())
		}
		if fake.solicitacao.HashRequisicao == "" || fake.solicitacao.Itens[0].Quantidade.String() != "2.5" {
			t.Fatalf("solicitacao=%+v", fake.solicitacao)
		}
	})
	...
	t.Run("erro ao ler corpo retorna 400", func(t *testing.T) {
		request := httptest.NewRequest(http.MethodPost, "/estoque/solicitacoes-compra-reserva-insumos", nil)
		request.Header.Set("Idempotency-Key", "40000000-0000-0000-0000-000000000001")
		request.Body = corpoComErro{}
		resposta := httptest.NewRecorder()
		NewSolicitarCompraEReservarInsumosHandler(application.NewSolicitarCompraEReservarInsumos(&processarRepositorioFake{})).ServeHTTP(resposta, request)
		if resposta.Code != http.StatusBadRequest {
			t.Fatalf("status=%d", resposta.Code)
		}
	})
	...
	t.Run("mapeia erros esperados", func(t *testing.T) {
		casos := []struct {
			...
			{"item ausente", application.ErrInsumoNaoEncontrado, http.StatusNotFound},
...
func executarProcessamento(fake *processarRepositorioFake, corpo, chave string) *httptest.ResponseRecorder {
	request := httptest.NewRequest(http.MethodPost, "/estoque/solicitacoes-compra-reserva-insumos", strings.NewReader(corpo))
	if chave != "" {
		request.Header.Set("Idempotency-Key", chave)
	}
	response := httptest.NewRecorder()
	NewSolicitarCompraEReservarInsumosHandler(application.NewSolicitarCompraEReservarInsumos(fake)).ServeHTTP(response, request)
	return response
}
