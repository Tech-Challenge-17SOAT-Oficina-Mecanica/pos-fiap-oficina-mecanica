...
func TestTempoExecucao(t *testing.T) {
	...
	_, _ = db.Exec(ctx, "DELETE FROM ordem_servico WHERE placa_veiculo = 'TMP1A23' AND (iniciada_em >= '2099-01-10'::timestamptz OR finalizada_em >= '2099-01-10'::timestamptz)")
	...
	inicio120 := time.Date(2099, time.January, 10, 9, 0, 0, 0, time.UTC)
	inicio240 := time.Date(2099, time.January, 11, 8, 0, 0, 0, time.UTC)
	if _, err = db.Exec(ctx, `INSERT INTO ordem_servico (id,cliente_id,veiculo_id,placa_veiculo,status,iniciada_em,finalizada_em) VALUES
		($1,$6,$7,'TMP1A23','FINALIZADA',$8::timestamptz,$8::timestamptz + INTERVAL '120 minutes'),
		($2,$6,$7,'TMP1A23','FINALIZADA',$9::timestamptz,$9::timestamptz + INTERVAL '240 minutes'),
		($3,$6,$7,'TMP1A23','EM_EXECUCAO',$8::timestamptz,NULL),
		($4,$6,$7,'TMP1A23','FINALIZADA',NULL,$8::timestamptz + INTERVAL '30 minutes'),
		($5,$6,$7,'TMP1A23','FINALIZADA',$8::timestamptz,NULL),
		($10,$6,$7,'TMP1A23','FINALIZADA',$8::timestamptz,$8::timestamptz - INTERVAL '30 minutes')`, os120, os240, osEmExecucao, osSemInicio, osSemFim, clienteID, veiculoID, inicio120, inicio240, osInconsistente); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	resposta = requisitar("/ordens-servico/tempos-execucao?dataInicio=2099-01-10&dataFim=2099-01-11&tamanho=1&pagina=1", token)
	var lista struct {
	...
	resposta = requisitar("/ordens-servico/tempos-execucao?dataInicio=2099-01-11&dataFim=2099-01-11", token)
