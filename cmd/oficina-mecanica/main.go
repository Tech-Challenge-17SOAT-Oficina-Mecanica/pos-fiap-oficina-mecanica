...
func main() {
	...
	criarOrdemServico := ordemServicoApplication.NewCriar(ordemServicoInfrastructure.NewPostgresRepository(db))
	...
	mux.Handle("POST /ordens-servico", segurancaPresentation.RequireScope(jwt, "os:escrever", ordemServicoPresentation.NewCriarHandler(criarOrdemServico)))
