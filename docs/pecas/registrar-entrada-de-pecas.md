---
documento: Refinamento de Requisitos — Registrar Entrada de Peças
dono: A definir
versao: 0.3
...
# Refinamento de Requisitos — Registrar Entrada de Peças

Este documento detalha a tarefa Registrar Entrada de Peças do contexto de Peças.

> **Escopo deste documento.** O recebimento é **uma rota só**, compartilhada com insumos. Este
> documento descreve a parte de **peças**; a de insumos está em [registrar-entrada-de-insumos.md](../insumos/registrar-entrada-de-insumos.md).
...
Registrar o recebimento de peças, aumentando o saldo físico do estoque e liberando para execução as
...
- A peça recebida deve estar cadastrada e ativa.
...
| ID | Requisito |
|---|---|
| RF-PEC-67 | Permitir registrar entrada informando item, quantidade, custo unitário e documento de origem. |
| RF-PEC-68 | Permitir registrar a entrada de vários itens em um mesmo recebimento. |
| RF-PEC-69 | Validar que a quantidade informada é maior que zero. |
| RF-PEC-70 | Vincular a entrada ao pedido de compra correspondente, quando houver. |
| RF-PEC-71 | Atualizar o saldo físico do item. |
| RF-PEC-72 | Efetivar as reservas vinculadas ao pedido de compra recebido. |
| RF-PEC-73 | Registrar a movimentação no histórico de estoque. |
| RF-PEC-74 | Atualizar a situação do pedido de compra conforme o recebimento seja parcial ou total. |
| RF-PEC-75 | Identificar as Ordens de Serviço vinculadas ao pedido recebido. |
| RF-PEC-76 | Alterar o status dessas OS de `AGUARDANDO_RECURSOS` para `AGUARDANDO_EXECUCAO` quando todos os seus itens pendentes estiverem atendidos. |
| RF-PEC-77 | Manter em `AGUARDANDO_RECURSOS` as OS que ainda possuem itens pendentes. |

**Requisitos Não Funcionais**

| ID | Requisito |
|---|---|
| RNF-PEC-43 | A operação deve ser feita por API RESTful. |
| RNF-PEC-44 | A operação deve ser acessível somente por usuário autorizado com permissão de estoque. |
| RNF-PEC-45 | A entrada deve ser transacional — ou todos os itens do recebimento entram, ou nenhum entra. |
| RNF-PEC-46 | A mudança de status das OS deve ocorrer na mesma operação da entrada. |
| RNF-PEC-47 | A operação deve ser idempotente em relação ao documento de origem, para impedir dupla contagem em caso de reenvio. |
| RNF-PEC-48 | A movimentação deve ser auditável e o histórico imutável. |
| RNF-PEC-49 | A operação deve ser protegida contra concorrência, sem perda de atualização de saldo. |
...
> `documentoOrigem`. Este documento descreve a parte de **peças** do recebimento; a de insumos está em
> [registrar-entrada-de-insumos.md](../insumos/registrar-entrada-de-insumos.md).
...
- `quantidade` maior que zero; **inteira** nas linhas do tipo `PECA` — não há fração de peça.
- `custoUnitario` maior que zero.
- Sem `itemId` repetido no mesmo payload.

*Negócio*

- Todos os `itemId` existem e estão ativos. A rota aceita peça e insumo no mesmo recebimento; as validações abaixo tratam das linhas do tipo `PECA`.
